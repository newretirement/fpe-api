...
| `goalMetric` | enum | When `algoType="heuristic"`, this attribute determines the intended goal of the optimization. Valid values are: [`estateValue`, `lifetimeTaxes`]. When `algoType` is set to something other than `heuristic`, this attribute is ignored. |
