...
| `goalMetric` | enum | When `algoType="heuristic"`, this attribute is required, and determines the goal of the heuristic optimization. Valid values are: [`estateValue`, `lifetimeTaxes`]. When `algoType="rulebased"`, this attribute is not required, and is not read from by the algorithm. |
