...
| `goalMetric` | enum | When `algoType="heuristic"`, this attribute is required, and determines the intended goal of the optimization. Valid values are: [`estateValue`, `lifetimeTaxes`]. When `algoType` is set to something other than `heuristic`, this attribute is not applicable, and is ignored. |
