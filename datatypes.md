...
| `useRuleBasedAlgo` | boolean | Determines the optimization algorithm to use.  Defaults to `false`.  Valid values:<br/><ul><li>`true` =  Use a rule-based algorithm (one that attempts to mimic what a human financial advisor might recommend) <li>`false` = Use a heuristic algorithm (i.e. one that attempts to "discover" the optimal set of Roth conversions)<ul/> |
